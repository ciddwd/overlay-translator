...
- **悬浮球长按**：弹出弧形子菜单，常用动作一指可达：循环翻译 / 重选区域 / 输入翻译 / 在「全屏翻译」和「划词翻译」之间切换 / 快速切源语言与目标语言 / 切换翻译预设 / 打开设置 / 回主页。**按钮顺序和每页数量可在设置里调整**；超过一页自动出现「下一组」翻页按钮
- **输入翻译**：单击操作球，将输入框里的文字翻译为预设的源语言；双击可选择全屏翻译或划词翻译
- **快捷开关**：在手机下拉快捷设置中，直接开启或关闭截屏服务
- **循环模式**：默认使用「停稳翻译」，画面停止变化后自动翻译。也支持固定间隔和等待文字显示完整后翻译。
- **音量双键**：同时按住「音量+ / 音量−」0.3 秒触发，手指不用离开游戏（需在系统无障碍中启用）
- **从其它 App 选中文字**：在任意 App 里长按选中一段文字 → 系统菜单选「屏译翻译」→ 直接弹译文卡片，省去切到屏译再框选的步骤

### 🔍 识别屏上的文字（OCR）

- **本机识别**：ML Kit（中日韩英）/ PaddleOCR / 日漫 OCR——图不上传，无网也能用；PaddleOCR 支持 v5/v6 多档模型，也可单独选择 PP-OCRv5 韩文
...
- **服务商快速配置**：可选择魔搭 ModelScope、OpenRouter、AIHubMix、302.AI、国家超算互联网等服务商；部分服务可选择中国大陆或国际地址
- **本机翻译**：Google ML Kit 下载语言包后可离线使用；Sakura 适合日语漫画和视觉小说翻简中，Hy-MT2 支持更多语言
- **语言方向保护**：源语言和目标语言不能设为同一种语言；设置页和悬浮快捷菜单会禁用冲突选项并说明原因
- **译名一致性**：可按全局或指定 App 保存人名、地点、组织和专业术语，支持单条添加和 JSON 批量导入；长按已经显示的译文还能修改原文或译文，并把纠正结果保存下来
...
- **小牛翻译**：可选择 Flash 或 Pro；Pro 支持边翻译边显示
...
- **预设记住密钥**：不同预设可在本机分别保存密钥，切换时一同切换；密钥始终不会被导出
- **英语离线词典**：导入英语 (ECDICT) 词典包后，可选择在本机查询词义
...
| **悬浮球** | **智能循环翻译** |
| <img src="docs/screenshots/settings-floating.png" width="280" alt="悬浮球设置" /> | <img src="docs/screenshots/settings-loop-translation.png" width="280" alt="智能循环翻译设置" /> |

| **划词翻译 / 截屏区域 / 输入翻译** | **TTS 朗读设置** |
|---|---|
| <img src="docs/screenshots/settings-word-select.png" width="280" alt="划词翻译 / 截屏区域 / 输入翻译" /> | <img src="docs/screenshots/settings-tts.png" width="280" alt="TTS 朗读引擎、系统音色、语速、音调和测试文本设置" /> |
...
- 在系统"无障碍"里启用本应用，**同时按下音量加 / 音量减并按住 300ms** 作为全局触发。屏译自身的主要悬浮控件提供 TalkBack 标签、状态和操作提示
...
- 在手机下拉快捷设置中添加屏译磁贴，即可直接开启或关闭截屏服务

### 输入翻译

在弧形菜单中切换到「输入翻译」，先点击需要翻译的输入框并输入文字：

- **单击操作球**：将输入框里的文字翻译为预设的源语言，并替换原内容
- **双击操作球**：执行全屏翻译或划词翻译，可在设置的「输入翻译」卡片中选择
...
| **端侧** PP-OCRv5 韩文 | 韩文文字 | 独立的韩文识别模型，可在设置中选择；首次使用需下载模型 |
...
| **小牛翻译** | 通用文本翻译 | API Key；Flash 还需填写 App ID | 可选择 Flash 或 Pro；Pro 支持边翻译边显示 |
...
- 导入前可预览、筛选和取消勾选；已有相同术语时，可选择跳过或覆盖
- 不同原文可以使用相同译名；同一原文填写了不同译名时，需要先选择保留哪条
- 单个文件最多 5,000 条，不超过 2 MB

### 词典库

在「翻译库 → 词典库」中管理查询方式：

- **在线查询**：使用当前配置的云端 LLM，需要选择 OpenAI 或 Anthropic 兼容的翻译引擎
- **离线查询**：导入适用于屏译的英语 (ECDICT) `.bin` 词典包后，在本机查询
- **点击原文显示词义**：可在此页面开启或关闭

### 循环翻译

- **触发方式**：默认「停稳翻译」，也可选择「固定间隔」或「智能等待文字完成」
- **停稳翻译**：翻页后自动翻译新页面，无需反复点击悬浮球。
...
- **自定义预设**：当前设置可保存为预设；只有和当前设置完全匹配时才显示为已应用，改过任意关键项会回到"未保存预设"状态。不同预设可在本机分别保存密钥，切换时一同切换；密钥始终不会被导出。
- **弧形菜单按钮**：可拖动排序，可选每页显示 2–6 个按钮；如果还有下一组，最后一格会自动变成「下一组」。菜单可包含循环、截图区域、输入翻译、划词/整屏切换、语言快切、预设快切、设置页和返回主应用。
...
| **多种 OCR** | 使用手机端 ML Kit / PaddleOCR / 日漫 OCR，可单独选择 PP-OCRv5 韩文；也能连接局域网 OCR 或百度、腾讯、有道、PP-OCRv6 在线服务 |
| **横排与竖排** | 自动识别旋转画面、横排和竖排日漫；译文可以跟随识别结果，也能手动指定排列和阅读方向 |
| **画面方向修正** | 用于画面方向与设备方向不一致的场景 |
| **多种翻译方式** | 使用 Google ML Kit 端侧翻译、OpenAI 兼容接口、DeepL、小牛翻译、Google、火山、百度、腾讯、有道图翻，或 Sakura / Hy-MT2 离线模型 |
| **服务商快速配置** | 可选择魔搭 ModelScope、OpenRouter、AIHubMix、302.AI、国家超算互联网等服务商；部分服务可选择中国大陆或国际地址 |
...
| **英语离线词典** | 导入英语 (ECDICT) 词典包后，可选择在本机查询词义 |
| **固定人名与术语** | 保存全局或指定游戏的人名、地点和专业术语，让连续剧情中的译名保持一致；支持 JSON 批量导入，导入前可预览、筛选和取消勾选 |
| **模型下载** | 在后台下载端侧模型，通过应用和通知查看进度，并可取消、失败重试或断点续传 |
| **本地翻译优化** | 启动截屏后提前准备离线模型，并尽量一起处理同一画面的多段文字，减少等待 |
| **个性化与迁移** | 调整悬浮球透明度，以及译文颜色、字体、大小、描边和阴影；导出设置、预设、术语和字体，换机后再导入 |
| **辅助功能** | 使用音量双键触发、TalkBack 操作提示、崩溃日志导出和国产手机后台运行引导 |
| **Shizuku 多版本兼容** | 支持 Sui、官方 Shizuku、Nightzuku、thedjchi/Shizuku 和 Shizuku+ 进行授权与截屏 |
...
如果打算自己实现某个 issue，麻烦提前在里面留言认领，避免重复投入。如果没有说明要接手，我一般就会根据自己的时间安排直接做。
