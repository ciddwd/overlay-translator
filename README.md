...
- **译文朗读（TTS）**：可朗读译文、划词结果或自己选中的一段文字；再次点击可暂停 / 继续，出错时会直接提示原因
...
| **自建语音服务** | 已经在电脑或局域网运行语音服务 | 填写服务地址后即可使用，适合希望自己管理服务的用户 |
| **火山引擎** | 已经开通火山语音服务 | 填写账号信息并选择音色；使用厂商服务可能产生费用 |
| **MiniMax** | 想使用更多音色，或自己复刻、设计音色 | 可以搜索、试听和选用音色，也能上传录音复刻声音，或用文字描述设计新音色 |
| **MiMo** | 已经使用 MiMo 语音服务 | 支持内置音色、复刻音色，也可以用文字说明想要的说话风格 |

- **怎么朗读**：点击译文或划词结果旁的播放按钮；选中一段横排译文后，也可以从弹出的菜单中选择“朗读”。竖排译文无法直接长按选择，可切换到“点击块选择复制”模式
- **暂停和继续**：再次点击同一段文字即可暂停，之后再点会从上次位置继续；点击另一段文字则改为朗读新内容
- **声音太小**：自建语音服务、火山、MiniMax 和 MiMo 可以增加 `0–24 dB` 音量。调得过高可能失真；手机系统语音不支持这项调节
